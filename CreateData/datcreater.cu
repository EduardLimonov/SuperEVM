	...
	int polySize, int* polygon)
{
	...
	for (int i = 0; i < nTriang + nCircl; i++)
	{
		...
		do
		{
			lt = { randUniform(start.x, start.x + d.x, state), randUniform(start.y, start.y + d.y, state) };
		...
		Rect nr(lt, rb);
		if (onPolygonEdge(nr, polySize) || atLeft(nr, start, end) && atRight(nr, start, end))
			continue;
		/*else if (onBorder(nr, start, end))
		{
			bool first = atLeft(nr, start, end), second = atRight(nr, start, end);
			if (first && second)
				continue;
			else if (first)
	...
	/*conflicts[blockIdx.x * gridDim.x + threadIdx.x].first.insert(
		conflicts[blockIdx.x * gridDim.x + threadIdx.x].first.end(),
		rects.begin(),
		rects.end()
	);*/

	syncAll(); // синхронизация всех потоков всех блоков
...
{	// удаляет из вектора все прямоугольники, которые пересекаются с предыдущими
	...
	{
		for (auto it_next = it + 1; it_next != v.end(); )
		{
			Rect r_n = *it_next;
			Rect rr = *it;
			if (rr.intersects(r_n))
				it_next = v.erase(it_next);
			else
				it_next++;
		}
	}
